...
class UProbeAPI:
    """
    A class to manage the U-Probe design workflow.
    """
    ...
    def __init__(
        ...
        """
        Initializes the U-Probe API.

        Args:
            protocol_config: Path to protocol config file or a dictionary.
            genomes_config: Path to genomes config file or a dictionary.
            output_dir: Directory to save output files.
        """
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(parents=True, exist_ok=True)

        log.info("--- Initializing U-Probe API ---")
    ...
    def _load_config(self, config: T.Union[Path, dict]) -> dict:
        """Loads a YAML configuration from a path or returns the dict."""
        if isinstance(config, Path):
            return yaml.safe_load(config.read_text(encoding="utf-8"))
        return config

    def _validate_and_get_genome(self) -> dict:
        """Validates genome configuration and file paths."""
    ...
    def build_genome_index(self, threads: int = 10):
        """Builds the genome index using STAR."""
    ...
    def generate_barcodes(self) -> T.Dict[str, T.List[str]]:
        """Generates barcodes based on the protocol configuration."""
    ...
    def validate_targets(self, continue_on_invalid: bool = False) -> bool:
        """
        Validates target genes against the GTF file.

        Args:
            continue_on_invalid: If True, continue with valid targets even if some are invalid.
        
        Returns:
            True if all targets are valid or if continuing with valid ones, False otherwise.
        """
    ...
    def generate_target_seqs(self) -> pd.DataFrame:
        """
        Generates target region sequences.

        Returns:
            A DataFrame with target sequences and metadata.
        """
    ...
    def construct_probes(self, df_targets: pd.DataFrame) -> pd.DataFrame:
        """
        Constructs probes from target sequences.

        Args:
            df_targets: DataFrame containing target sequences.

        Returns:
            A DataFrame with constructed probes.
        """
    ...
    def post_process_probes(self, df_probes: pd.DataFrame, raw_csv: bool = False) -> pd.DataFrame:
        """
        Adds attributes and applies post-processing filters to probes.

        Args:
            df_probes: DataFrame with probes to process.
            raw_csv: If True, save the unfiltered, attributed probes to a CSV file.

        Returns:
            A DataFrame with final, processed probes.
        """
    ...
    def run_workflow(self, raw_csv: bool = False, continue_on_invalid_targets: bool = False, threads: int = 10) -> pd.DataFrame:
        ...
        Args:
            raw_csv: If True, save intermediate raw probe data.
            continue_on_invalid_targets: If True, proceed even if some targets are invalid.
            threads: Number of threads for genome indexing.

        Returns:
            A DataFrame with the final, processed probes.
        """
